  ...
  try {
    const posts = await Post.find()
      ...
      .populate({
        path: "user",
        select: "-password",
      }) //createdAt: -1 [means all posts created new also] populate means to get all the data of user in particular post excludes password
      .populate({
        path: "comments.user",
        select: "-password",
      });
...
export const getFollowingPosts = async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ error: "Internal Server Error" });
    console.log("Error while getting following posts ", error.message);
  }
};
...
export const likeUnlikePost = async (req, res) => {
  try {
    ...
    if (userLikedPost) {
      ...
      return res.status(200).json({ error: "Post liked sucessfully" });
    }
  } catch (error) {
    res.status(500).json({ error: "Internal Server Error" });
